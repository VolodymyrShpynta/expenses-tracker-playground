...
- **The Keycloak account itself.** Erasure must cascade to deleting the user in
  ...
  change.
- **The mobile sync file**, if the user is also a mobile user. The backend cannot
  reach it and, more importantly, has no business reaching across into a separate
  application's data set: the mobile app keeps its data entirely off the backend
  (local SQLite + the user's own cloud-drive sync file, of which Google / Microsoft
  is the user's sub-processor, not the operator's). **Mobile-side erasure is the
  mobile app's own responsibility** and belongs in that app's settings screen
  (wipe local DB + delete the cloud sync file). The backend's web erasure response
  must **not** mention it, because the web flow has no way to know whether the
  caller is also a mobile user and any such hint would leak a false cross-module
  coupling.
- **The web app's `localStorage` preferences**, which are keyed by `user_id` and
  will linger until the browser clears them. Negligible but worth noting — and
  this one **does** belong in the web erasure response's follow-up checklist.
