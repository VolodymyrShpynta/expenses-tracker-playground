...
/**
 * Default (business) implementation of [CategoryService].
 *
 * Knows nothing about GDPR — Art. 18 restriction enforcement is the
 * sole responsibility of the `GdprAwareCategoryService` decorator,
 * which is the `@Primary` bean and the one controllers receive when
 * they inject [CategoryService].
 *
 * [mergeInto] injects the *default* [ExpenseCommandService] via
 * `@Qualifier("defaultExpenseCommandService")` rather than the
 * `@Primary` decorator: a merge can touch hundreds of expenses in a
 * single transaction, and re-checking the restriction guard per
 * iteration would issue one redundant `processing_restrictions` PK
 * lookup per expense. The outer call into `mergeInto` has already been
 * guarded by `GdprAwareCategoryService`, so the contract "every entry
 * from outside the service layer is guarded exactly once" is
 * preserved.
 */
...
class DefaultCategoryService(
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(DefaultCategoryService::class.java)
    }

    /**
     * Returns the user's full catalog (active + soft-deleted) and
     * triggers first-time default seeding when the user has no rows
     * yet. The count aggregate is collected up-front (small — one row
     * per category referenced by an active expense) so the resulting
     * Flow can stream categories lazily without a per-row count query.
     */
    override fun findAllCategoriesWithExpenseCounts(): Flow<Pair<Category, Long>> = flow {
     ...
     * rolls back the entire merge so the user never observes a
     * half-merged state. The inner `@Transactional` on
     * `DefaultExpenseCommandService.updateExpense` joins this
     * transaction (`PROPAGATION_REQUIRED`). The injected
     * `expenseCommandService` is the `default` bean (not the GDPR
     * decorator) so the per-iteration guard re-check is avoided — see
     * the class-level doc.
     */
    @Transactional
     ...
     *
     * Wrapped in a single `@Transactional` boundary so a partial
     * failure (e.g. DB error halfway through the upsert loop) leaves no
     * orphan state. The inner `@Transactional` on
     * [DefaultCategorySeeder.seedDefaultCategories] joins this
     * transaction (`PROPAGATION_REQUIRED`), keeping a single rollback
     * point.
     *
